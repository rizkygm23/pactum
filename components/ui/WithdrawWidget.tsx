...
export function WithdrawWidget({ expectedMerchantAddress }: { expectedMerchantAddress: string | null }) {
      ...
      if (typeof window !== "undefined" && window.ethereum) {
        try {
          ...
          if (accounts && accounts.length > 0) {
            // Auto-switch to Arc — a wallet parked elsewhere breaks the withdraw tx
  ...
  const handleWithdraw = async () => {
    ...
    try {
      ...
      toast.error("Arc Testnet must be selected in your wallet to withdraw.");
      return;
    }
  ...
  if (!expectedMerchantAddress) {
    return (
      <div className="bg-canvas p-4 sm:p-6 rounded-lg border border-hairline flex items-center justify-between">
        <div className="min-w-0">
          <h3 className="text-sm font-semibold text-ink">Withdraw to Wallet</h3>
          <p className="text-xs text-slate mt-1">You have not registered a Settlement Wallet in Settings.</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="bg-canvas p-4 sm:p-6 rounded-lg border border-hairline mt-6 relative overflow-hidden">
      {/* Decorative gradient */}
      <div className="absolute top-0 right-0 w-32 h-32 bg-hairline rounded-full blur-3xl -mr-10 -mt-10 pointer-events-none"></div>
      
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 sm:gap-6 relative z-10">
        <div className="min-w-0">
          <h3 className="text-base sm:text-lg font-semibold text-ink flex items-center gap-2">
            <Wallet className="w-5 h-5 text-ink shrink-0" />
            Withdraw to Wallet
          </h3>
        ...
        <div className="flex w-full flex-col gap-3 md:w-auto md:min-w-[200px] md:items-end">
          {isInitializing ? (
             <div className="flex items-center gap-2 text-slate text-sm py-2.5">
               <Loader2 className="w-4 h-4 animate-spin" /> Checking wallet...
             </div>
          ) : !address ? (
            <button
              onClick={connectWallet}
              className="btn-ghost focus-ring no-wrap text-sm w-full md:w-auto"
            >
              <Wallet className="w-4 h-4 text-slate" /> Connect Wallet
            </button>
          ) : (
            <div className="flex flex-col md:items-end w-full">
              <div className="flex items-center gap-2 mb-2 bg-canvas-warm px-3 py-1.5 rounded-md border border-hairline">
                <div className={`w-2 h-2 rounded-full ${address.toLowerCase() === expectedMerchantAddress.toLowerCase() ? 'bg-green-500' : 'bg-red-500'}`}></div>
                <span className="text-xs font-mono text-slate no-wrap">{address.slice(0, 6)}...{address.slice(-4)}</span>
                <button onClick={switchWallet} className="text-xs text-ink hover:text-ink/80 ml-auto underline decoration-hairline-soft underline-offset-2">Switch</button>
              </div>
              
              <div className="flex flex-wrap items-center gap-3 sm:gap-4 w-full justify-between md:justify-end">
                <div className="min-w-0 text-left md:text-right">
                  <p className="text-xs text-slate mb-0.5 no-wrap">Ready to Withdraw</p>
                  <p className="text-lg sm:text-xl font-bold text-ink break-all">{withdrawableBalance.toFixed(6)} <span className="text-sm text-slate font-normal">USDC</span></p>
                </div>
              ...
              {status && (
                <p className={`text-xs mt-3 max-w-full break-words md:max-w-xs md:text-right ${status.includes('Gagal') || status.includes('Failed') ? 'text-ink' : 'text-ink'}`}>
                  {status}
                </p>
              )}
            </div>
